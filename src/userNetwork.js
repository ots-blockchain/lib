...
import { Transaction, Block } from './models.js';
...
export default class P2PNetwork {
     ...
     * @param {Function} [callback] tx callback
     */
    constructor(privateKey, callback) {
    ...
    connectToPeer(url) {
        const connect = () => {
            if (this.socket) {
                ...
                if (this.socket.readyState === 1 || (typeof WebSocket !== 'undefined' && this.socket.readyState === WebSocket.OPEN)) {
                    this.socket.close();
                }
                this.socket = null;
            }
            ...
            socket.onopen = () => {
                console.info(`Connected to peer: ${url}`);
                this.initConnection();
            };
    ...
    initConnection() {
        ...
        this.socket.onmessage = async (event) => await this.handleMessage(event.data);
    }
    ...
    async closeConnection(connect, reconnect = true) {
        if (reconnect && connect) {
            console.warn(`Peer disconnected. Retrying in ${consts.RECONNECT_INTERVAL / 1000}s...`);
    ...
    async getNonce(address) {
        ...
        for (let i = 0; i < consts.MAX_RETRIES; i++) {
            try {
                ...
                lastError = err;
                console.warn(`getNonce attempt ${i + 1}/${consts.MAX_RETRIES} failed: ${err.message}`);
    ...
    async getBlock(height) {
        ...
        for (let i = 0; i < consts.MAX_RETRIES; i++) {
            try {
                ...
                lastError = err;
                console.warn(`getBlock attempt ${i + 1}/${consts.MAX_RETRIES} failed: ${err.message}`);
                if (i < consts.MAX_RETRIES - 1) {
                    await new Promise(r => setTimeout(r, 500));
                }
            }
    ...
    async getInfo() {
        ...
        for (let i = 0; i < consts.MAX_RETRIES; i++) {
            try {
                return await this._requestWithTimeout('INFO', null, consts.REQUEST_TIMEOUT);
            } catch (err) {
                lastError = err;
                console.warn(`getInfo attempt ${i + 1}/${consts.MAX_RETRIES} failed: ${err.message}`);
                if (i < consts.MAX_RETRIES - 1) {
                    await new Promise(r => setTimeout(r, 500));
                }
            }
    ...
    async getAccount(address) {
        ...
        for (let i = 0; i < consts.MAX_RETRIES; i++) {
            try {
                ...
                lastError = err;
                console.warn(`getAccount attempt ${i + 1}/${consts.MAX_RETRIES} failed: ${err.message}`);
                if (i < consts.MAX_RETRIES - 1) {
                    await new Promise(r => setTimeout(r, 500));
                }
            }
    ...
    async getStorage(address, key) {
        ...
        for (let i = 0; i < consts.MAX_RETRIES; i++) {
            try {
                ...
                lastError = err;
                console.warn(`getStorage attempt ${i + 1}/${consts.MAX_RETRIES} failed: ${err.message}`);
                if (i < consts.MAX_RETRIES - 1) {
                    await new Promise(r => setTimeout(r, 500));
                }
            }
    ...
    async handleMessage(rawData) {
        ...
        if (typeof Buffer !== 'undefined' && Buffer.isBuffer(rawData)) {
            ...
            binaryData = new Uint8Array(rawData);
        } else if (rawData?.buffer instanceof ArrayBuffer) {
            binaryData = new Uint8Array(rawData.buffer);
        } else {
            return;
        }
        ...
        if (data?.type === 'TX_RES' && typeof this.callback === 'function') {
            this.callback(CryptoUtils.deserializeWithBigInt(data?.payload?.data));
        }
    ...
    async send(data) {
        ...
        if (!this.socket || (this.socket.readyState !== 1 && (typeof WebSocket !== 'undefined' && this.socket.readyState !== WebSocket.OPEN))) {
